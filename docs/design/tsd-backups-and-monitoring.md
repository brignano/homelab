...
- **Monitoring** — self-hosted **Healthchecks** (SQLite, new `checks.home`) for heartbeats; **alert via the Discord `#alerts` webhook**. Each job pings on success; Healthchecks alerts on a late/missing ping.
...
flowchart LR
    subgraph LXC["CT 100 docker @ 10.0.0.201"]
        ...
        NT["Discord #alerts webhook (existing)"]
    end
    jobs -->|curl ping on success| HC
    HC -->|late/missing ping| NT --> DEV[Discord · phone / desktop]
...
## Cost & footprint
- Healthchecks (~150–250 MB; alerting is an existing webhook, so nothing else to run) — ~$0, well within headroom (RAM ~45% used, CPU ~2%).
...
- Alerting: **Discord `#alerts` webhook only** (amended 2026-08-24; originally ntfy via `alerts.home`, since removed).
